...
class BiLstmContext(chainer.Chain):
    ...
    def __init__(self, deep, gpu, word2index, in_units, hidden_units, out_units, loss_func, train, drop_ratio=0.0):
        ...
        if deep:
            super(BiLstmContext, self).__init__(
                ...
                l3_1 = L.Linear(2*hidden_units, 2*hidden_units),
    ...
    def __call__(self, sent,tags):
        '''
        Train the network
        :param sent: a minibatch of sentences
        '''
        self.reset_state()
        return self._calculate_loss(sent,tags)
    ...
    def context_rep(self, sent, position):
        ...
        return sent_y[0][position].data[0]           
        ...
        if self.deep:    
            ...
            for bi_h in sent_bi_h:
                if self.drop_ratio > 0.0:
                    ...
                    y = self.l4(h1)
                    h2 = F.relu(self.l3_1(bi_h))
                    z = self.l4(h2)
                sent_y.append(y)
                sent_z.append(z)
            return sent_y , sent_z
        else:
            return sent_bi_h

               
    def _calculate_loss(self, sent,tags):
        # sent is a batch of sentences.
        sent_arr = self.xp.asarray(sent, dtype=np.int32)

        sent_y , sent_z  = self._contexts_rep(sent_arr)
        ...
        tag_arr = self.xp.asarray(tags, dtype=np.int32)
        sent_t = []
        for i in range(tag_arr.shape[1]):
            t = chainer.Variable(tag_arr[:,i])
            sent_t.append(t)    
        
        # add POS loss
        for z,t in zip(sent_z, sent_t):
