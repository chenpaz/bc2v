...
from context2vec.train.sentence_reader import SentenceReaderDir
...
for epoch in range(args.epoch):
    ...
    reader.open()    
    for sent,tags in reader.next_batch():
        if len(tags) == 0: # got empty sent continue
            continue
        model.zerograds()
        loss = model(sent,tags)
